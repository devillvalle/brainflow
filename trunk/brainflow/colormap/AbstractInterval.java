...
public abstract class AbstractInterval implements Interval {
    ...
    public AbstractInterval(double _min, double _max) {
        assert _max >= _min : "Interval() : max must be greater than min";
        max = _max;
        min = _min;
    }
    ...
    public abstract Interval truncate(double newMaximum);
    //public abstract Interval suture(Interval other);


    public boolean overlapsWith(Interval other) {
        if (containsValue(other.getMinimum()) || containsValue(other.getMaximum())) {
            return true;
        }
