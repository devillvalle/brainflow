...
public final class IntervalColorModel implements Serializable {
    ...
    public IntervalColorModel resampleMap(int mapSize) {
        ...
        return new IntervalColorModel(new DoubleRange(displayRange.getMinimumDouble(), displayRange.getMaximumDouble()), nmodel, getName() +
                "ressampled(" + mapSize + ")");
    ...
    public Range getDisplayRange() {
        return new DoubleRange(displayRange.getMinimumDouble(), displayRange.getMaximumDouble());
    }
    ...
    public IntervalColorModel safeCopy() {
        IntervalColorModel nmodel = new IntervalColorModel(new DoubleRange(displayRange.getMinimumDouble(), displayRange.getMaximumDouble()), wrappedModel, getName());
    ...
    public IntervalColorModel makeAndCopy(Color constantColor, String name) {
        ...
        IntervalColorModel nmodel = new IntervalColorModel(new DoubleRange(displayRange.getMinimumDouble(), displayRange.getMaximumDouble()), icm, getName());
        return nmodel;

    }

    public IntervalColorModel makeAndCopy(IndexColorModel _wrappedModel, String name) {
        IntervalColorModel nmodel = new IntervalColorModel(new DoubleRange(displayRange.getMinimumDouble(), displayRange.getMaximumDouble()), _wrappedModel, getName());
        return nmodel;
    }

    public final int indexOf(double value) {
        double intervalSize = displayRange.getMaximumDouble() - displayRange.getMinimumDouble();
        int mapSize = wrappedModel.getMapSize();
        int nbin = (int)(((value-displayRange.getMinimumDouble())/intervalSize)*mapSize);
        if (nbin >= mapSize)
            nbin = mapSize-1;
        else if (nbin < 0)
            nbin=0;
    ...
    public static IntervalColorModel createFromXml(String rgbaXml) throws BrainflowException {
        ...
        try {
            ...
            ret = new IntervalColorModel(new DoubleRange(0,256), cm, name);
            //ret.setName(name);
    ...
    public static void main(String[] args) {
        ...
        IntervalColorModel icm = new IntervalColorModel(new DoubleRange(0,256),  tmp, "spectrum");
